...
const content: Content = {
  ...
  toolsAndOther: {
    title: {ru: 'Инструменты и прочее', en: 'Tools and other'},
    items: {
...
<template>
  <section class="stack">
    <h2 class="stack__title">{{ content.stack.title[language] }}</h2>
    <ul class="stack__list">
      <ItemStack v-for="(item, itemId) in content.stack.items" :item="item" class="stack__item"
                 :key="`stack_${itemId}`"/>
    </ul>
    <h3 class="stack__subtitle">{{ content.toolsAndOther.title[language] }}:</h3>
    <ul class="stack__list">
      <ItemStack v-for="(item, itemId) in content.toolsAndOther.items" :item="item" class="stack__item"
                 :key="`tool_${itemId}`"/>
    </ul>
  </section>
</template>
